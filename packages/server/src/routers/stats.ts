...
export const stats = router({
    ...
    getDailyStreak: privateProcedure.query(async ({ ctx }) => {
        const sql = await ctx.prisma.$queryRaw`
            with streak_data as (select date,
                            "userId",
                            case
                                when date(date + interval '1 day') = lag(date) over (partition by "userId" order by date desc) is null
                                    then true
                                else date(date + interval '1 day') = lag(date) over (partition by "userId" order by date desc)
                                end as x
                     from "UserDailyStatistic"
                     where "userId" = ${ctx.userId}
                       and (
                         xp > 0 or points > 0 or "fiveLetterWordGuessed" = true
                         )
                     order by date desc)
select count(*), min(date) as start_date, max(date) as end_date
from streak_data
where date > (select max(date)
        ...
        const startDate = sql[0].start_date
        const endDate = sql[0].end_date
        const streakCount =
            DateTime.fromJSDate(endDate).diffNow().negate().as("days") > 1
                ? 0
